...
MODULE_NAME = os.environ.get("MODULE_NAME", "module_D_range_surge")
MODULE_VERSION = os.environ.get("MODULE_VERSION", "1.1.0")

LOOKBACK_BARS = int(os.environ.get("LOOKBACK_BARS", "40"))
RANGE_MULTIPLIER = float(os.environ.get("RANGE_MULTIPLIER", "2.5"))
...
CLOSE_TO_EXTREME_MAX_PCT = float(os.environ.get("CLOSE_TO_EXTREME_MAX_PCT", "0.15"))
SMA_SHORT = int(os.environ.get("SMA_SHORT", "20"))
SMA_LONG = int(os.environ.get("SMA_LONG", "50"))
ATR_LEN = int(os.environ.get("ATR_LEN", "14"))
ATR_MULT = float(os.environ.get("ATR_MULT", "0.8"))
...
def detect_signals(snapshot: Dict[str, Any], mod_state: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for symbol, bars in symbols_map.items():
        ...
        bodies = [body_size(float(x["open"]), float(x["close"])) for x in window]
        ...
        if direction == "long" and not (last_close >= s_short >= s_long):
            continue
        if direction == "short" and not (last_close <= s_short <= s_long):
            continue

        if direction == "long":
            dist_to_high = (last_high - last_close) / last_range if last_range > 0 else 1.0
            if dist_to_high > CLOSE_TO_EXTREME_MAX_PCT:
                continue
        else:
            dist_to_low = (last_close - last_low) / last_range if last_range > 0 else 1.0
            if dist_to_low > CLOSE_TO_EXTREME_MAX_PCT:
                continue

        if not (last_range >= ATR_MULT * atr_val):
            continue
        ...
        key = f"{MODULE_NAME}:{INTERVAL_STRING}:{symbol}:{direction}"
...
def fetch_module_state() -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"[module D] state_fetch_failed: {exc}")
        return {}
...
def persist_module_state(mod_state: Dict[str, Any]) -> None:
    ...
    try:
        ...
        print(f"[module D] state_persist_failed: {exc}")
