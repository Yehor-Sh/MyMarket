...
class BearishEngulfingStrategy(ModuleBase):
    ...
    def __init__(self, client: BinanceClient) -> None:
        super().__init__(client, name="BearishEngulfingStrategy", abbreviation="BENGF+",
    ...
    def process_with_timeframes(self, symbol: str, primary_candles: Sequence[Kline], extra_candles: Dict[str, Sequence[Kline]]) -> Iterable[Signal]:
        ...
        if _is_bull(c1) and _is_bear(c2) and c2.close <= c1.open and (_body(c2)/(atr_val or 1e-9)) >= 0.28 and e20 and e50 and e20 < e50:
            return [self.make_signal(symbol, "SHORT", confidence=_conf(_body(c2)/(atr_val or 1e-9)), metadata={"pattern":"bearish_engulfing"})]
        return []
