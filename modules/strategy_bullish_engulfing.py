...
class BullishEngulfingStrategy(ModuleBase):
    class Cfg:
        ...
        atr_len = 14
        min_body_atr = 0.4
        soft_gap_eps = 0.002  # 0.2% tolerance for "gaps" on crypto

    def __init__(self, client: BinanceClient) -> None:
        super().__init__(client, name="Bullish Engulfing+ATR", abbreviation="BENG+", interval=self.Cfg.interval, lookback=self.Cfg.lookback)
    ...
    def process(self, symbol: str, candles: Sequence[Kline]) -> Iterable[Signal]:
        ...
        # Engulfing with soft gaps
        cond1 = _is_bear(c1)
        soft_gap_ok = c2.open <= c1.close * (1 + self.Cfg.soft_gap_eps)
        engulf = c2.close >= c1.open and _is_bull(c2)
