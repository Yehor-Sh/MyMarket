...
class BullishEngulfingStrategy(ModuleBase):
    ...
    def process_with_timeframes(
        ...
        sig = (# Last two candles
c1, c2 = candles[-2], candles[-1]
# Requirements
cond_prev_bear = _is_bear(c1)
engulf = _is_bull(c2) and c2.close >= c1.open and c2.open <= c1.close
size_ok = (_body(c2) / (atr_val or 1e-9)) >= 0.35
ema_trend = e20 and e50 and e20 > e50
if cond_prev_bear and engulf and size_ok and ema_trend:
    strength = (_body(c2) / (atr_val or 1e-9))
    conf = _confidence(strength)
    return Signal(symbol=symbol, side="LONG", strategy=self.abbreviation, confidence=conf, metadata={"pattern":"bullish_engulfing"})
return None
)
        if sig is None:
            return []
