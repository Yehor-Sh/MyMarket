...
class DarkCloudCoverStrategy(ModuleBase):
    ...
    def __init__(self, client: BinanceClient) -> None:
        super().__init__(client, name="DarkCloudCoverStrategy", abbreviation="DCC+",
    ...
    def process_with_timeframes(self, symbol: str, primary_candles: Sequence[Kline], extra_candles: Dict[str, Sequence[Kline]]) -> Iterable[Signal]:
        ...
        if _is_bull(c1):
            pen = c1.open + _body(c1)*0.5
            if _is_bear(c2) and c2.close < pen and (_body(c2)/(atr_val or 1e-9)) >= 0.25 and e20 and e50 and e20 > e50:
                return [self.make_signal(symbol, "SHORT", confidence=_conf(_body(c2)/(atr_val or 1e-9)), metadata={"pattern":"dark_cloud_cover"})]
        return []
