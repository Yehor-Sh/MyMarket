...
shape = lower >= 1.5*body and upper <= 0.6*body
close_ok = _is_bull(c)
size_ok = (body / (atr_val or 1e-9) >= 0.2) or (lower / (atr_val or 1e-9) >= 0.5)
ema_down = e20 and e50 and e20 < e50
if shape and close_ok and size_ok and ema_down and c.low <= min(prev.low, candles[-3].low):
    strength = lower / (atr_val or 1e-9)
    conf = _confidence(strength)
    return Signal(symbol=symbol, side="LONG", strategy=self.abbreviation, confidence=conf, metadata={{"pattern":"hammer", "initial_sl_atr": 1.5, "tp_atr": 2.0}})
return None
)
        if sig is None:
            return []
