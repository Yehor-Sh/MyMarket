...
class HammerStrategy(ModuleBase):
    ...
    def __init__(self, client: BinanceClient) -> None:
        super().__init__(client, name="HammerStrategy", abbreviation="HAM+",
    ...
    def process_with_timeframes(self, symbol: str, primary_candles: Sequence[Kline], extra_candles: Dict[str, Sequence[Kline]]) -> Iterable[Signal]:
        ...
        if lower >= 1.3*body and upper <= 0.7*body and _is_bull(c) and (body/(atr_val or 1e-9) >= 0.18 or lower/(atr_val or 1e-9) >= 0.45) and e20 and e50 and e20 < e50 and c.low <= min(prev.low, candles[-3].low):
            return [self.make_signal(symbol, "LONG", confidence=_conf(lower/(atr_val or 1e-9)), metadata={"pattern":"hammer"})]
        return []
