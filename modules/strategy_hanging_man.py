...
if shape and close_ok and size_ok and ema_up and c.high >= max(prev.high, candles[-3].high):
    strength = lower / (atr_val or 1e-9)
    conf = _confidence(strength)
    return Signal(symbol=symbol, side="SHORT", strategy=self.abbreviation, confidence=conf, metadata={"pattern":"hanging_man"})
return None
)
        if sig is None:
            return []
