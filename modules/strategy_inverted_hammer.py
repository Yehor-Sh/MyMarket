...
shape = upper >= 1.6*body and lower <= 0.5*body
close_ok = _is_bull(c)
size_ok = max(body, upper) / (atr_val or 1e-9) >= 0.35
ema_down = e20 and e50 and e20 < e50
if shape and close_ok and size_ok and ema_down:
    strength = upper / (atr_val or 1e-9)
    conf = _confidence(strength)
    return Signal(symbol=symbol, side="LONG", strategy=self.abbreviation, confidence=conf, metadata={"pattern":"inverted_hammer"})
return None
)
        if sig is None:
            return []
