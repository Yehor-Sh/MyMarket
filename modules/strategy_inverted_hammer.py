...
class InvertedHammerStrategy(ModuleBase):
    ...
    def __init__(self, client: BinanceClient) -> None:
        super().__init__(client, name="InvertedHammerStrategy", abbreviation="IHAM+",
    ...
    def process_with_timeframes(self, symbol: str, primary_candles: Sequence[Kline], extra_candles: Dict[str, Sequence[Kline]]) -> Iterable[Signal]:
        ...
        if upper >= 1.4*body and lower <= 0.6*body and _is_bull(c) and e20 and e50 and e20 < e50 and max(body, upper)/(atr_val or 1e-9) >= 0.3:
            return [self.make_signal(symbol, "LONG", confidence=_conf(upper/(atr_val or 1e-9)), metadata={"pattern":"inverted_hammer"})]
        return []
