...
confirm = _is_bull(c3) and c3.close > (c1.open + c1.close)/2 and c3.close > c2.close
size_ok = (_body(c3) / (atr_val or 1e-9)) >= 0.35
ema_down = e20 and e50 and e20 < e50
if small_mid and confirm and size_ok and ema_down:
    strength = _body(c3) / (atr_val or 1e-9)
    conf = _confidence(strength)
    return Signal(symbol=symbol, side="LONG", strategy=self.abbreviation, confidence=conf, metadata={"pattern":"morning_star"})
return None
)
        if sig is None:
            return []
