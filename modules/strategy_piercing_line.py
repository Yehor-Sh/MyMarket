...
if not _is_bear(c1):
    return None
body1 = _body(c1)
penetration = c1.open - body1*0.4  # 60% into c1 body
cond = _is_bull(c2) and c2.close >= penetration
size_ok = (_body(c2) / (atr_val or 1e-9)) >= 0.3
ema_down = e20 and e50 and e20 < e50
if cond and size_ok and ema_down:
    strength = _body(c2) / (atr_val or 1e-9)
    conf = _confidence(strength)
    return Signal(symbol=symbol, side="LONG", strategy=self.abbreviation, confidence=conf, metadata={"pattern":"piercing_line"})
return None
)
        if sig is None:
            return []
