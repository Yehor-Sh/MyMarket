...
class RelativeVolume4H(ModuleBase):
    ...
    def __init__(self, client: BinanceClient) -> None:
        super().__init__(client, name="RelativeVolume4H", abbreviation="RVOL4H",
    ...
    def process(self, symbol: str, candles: Sequence[Kline]) -> Iterable[Signal]:
        ...
        if last.volume >= 2.0*(avg20 or 1e-9):
            if _is_bull(last):
                m = dict(meta); m.update({"pattern":"RVOL4H","avg20":avg20,"unclosed":True})
                return [self.make_signal(symbol, "LONG", confidence=_conf(last.volume/((avg20 or 1e-9)*2.0)), metadata=m)]
            if _is_bear(last):
                m = dict(meta); m.update({"pattern":"RVOL4H","avg20":avg20,"unclosed":True})
                return [self.make_signal(symbol, "SHORT", confidence=_conf(last.volume/((avg20 or 1e-9)*2.0)), metadata=m)]
        return []
