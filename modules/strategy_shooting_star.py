...
class ShootingStarStrategy(ModuleBase):
    class Cfg:
        interval = "15m"
        lookback = 60
        ema_fast = 20
    ...
    def __init__(self, client: BinanceClient) -> None:
        super().__init__(client, name="Shooting Star+", abbreviation="SSTAR+", interval=self.Cfg.interval, lookback=self.Cfg.lookback)
    ...
    def process(self, symbol: str, candles: Sequence[Kline]) -> Iterable[Signal]:
        ...
        cond_body = upper_shadow >= self.Cfg.shadow_mult * body and lower_shadow <= self.Cfg.low_shadow_max * body
        cond_close = _is_bear(latest)  # не требуем ниже prev.close
        cond_high = latest.high >= prev.high

        size_ok = max(body, upper_shadow) / atr >= 0.4

        if cond_body and cond_close and cond_high and size_ok:
