...
class SnapbackEMA4H(ModuleBase):
    ...
    def __init__(self, client: BinanceClient) -> None:
        super().__init__(client, name="SnapbackEMA4H", abbreviation="SNAP4H", interval=self.Cfg.interval, lookback=self.Cfg.lookback)
    ...
    def process(self, symbol: str, candles: Sequence[Kline]) -> Iterable[Signal]:
        ...
        if e20:
            dist_pct = (last.close - e20)/e20
        else:
            dist_pct = 0.0
        if dist_pct >= 0.03 and _is_bear(last):
            conf = _confidence((abs(dist_pct)*100)/5.0)
            m = dict(meta); m.update({"pattern":"SNAP4H","ema20":e20,"dist_pct":dist_pct,"unclosed":True})
            return [Signal(symbol=symbol, side="SHORT", strategy=self.abbreviation, confidence=conf, metadata=m)]
        if dist_pct <= -0.03 and _is_bull(last):
            conf = _confidence((abs(dist_pct)*100)/5.0)
            m = dict(meta); m.update({"pattern":"SNAP4H","ema20":e20,"dist_pct":dist_pct,"unclosed":True})
