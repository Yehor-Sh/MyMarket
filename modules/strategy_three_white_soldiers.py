...
class ThreeWhiteSoldiersStrategy(ModuleBase):
    class Cfg:
        ...
        min_rel_vol = 0.9
        # HTF filters
        rsi1h_min = 45 if "LONG" == "LONG" else 0
        rsi1h_max = 100 if "LONG" == "LONG" else 55
    ...
    def process_with_timeframes(
        ...
        sig = (c1, c2, c3 = candles[-3], candles[-2], candles[-1]
...
size_ok = (avg_body / (atr_val or 1e-9)) >= 0.28 and max(_body(c1), _body(c2), _body(c3)) / (atr_val or 1e-9) < 1.5
ema_down_before = e20 and e50 and ema([x.close for x in candles[:-3]], 20)[-1] < ema([x.close for x in candles[:-3]], 50)[-1]
if cond_bullish and cond_progress and within2 and within3 and size_ok and ema_down_before:
    strength = avg_body / (atr_val or 1e-9)
    conf = _confidence(strength)
    return Signal(symbol=symbol, side="LONG", strategy=self.abbreviation, confidence=conf, metadata={"pattern":"three_white_soldiers"})
return None
)
        if sig is None:
            return []
