...
class ThreeWhiteSoldiersStrategy(ModuleBase):
    ...
    def __init__(self, client: BinanceClient) -> None:
        super().__init__(client, name="ThreeWhiteSoldiersStrategy", abbreviation="TWS+",
    ...
    def process_with_timeframes(self, symbol: str, primary_candles: Sequence[Kline], extra_candles: Dict[str, Sequence[Kline]]) -> Iterable[Signal]:
        ...
        if _is_bull(c1) and _is_bull(c2) and _is_bull(c3) and c2.close > c1.close and c3.close > c2.close:
            avg = (_body(c1)+_body(c2)+_body(c3))/3.0
            tol = (atr_val or 0.0)*0.25
            w2 = (min(c1.open,c1.close)-tol) <= c2.open <= (max(c1.open,c1.close)+tol)
            w3 = (min(c2.open,c2.close)-tol) <= c3.open <= (max(c2.open,c2.close)+tol)
            if avg/(atr_val or 1e-9) >= 0.26 and w2 and w3 and e20 and e50 and e20 < e50:
                return [self.make_signal(symbol, "LONG", confidence=_conf(avg/(atr_val or 1e-9)), metadata={"pattern":"three_white_soldiers"})]
        return []
